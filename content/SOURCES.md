...
## Docs pages — `docs/`, an index plus nine pages, all generated

`docs/index.html` lists the nine, each card carrying the opening of that skill's own
`description` (verbatim, elided with an ellipsis where it runs long) and the lanes it sits on.
It is reachable from the header nav (*the pages*), from the landing footer, from every card
title, and from each page's breadcrumb — the nine pages existed from C5 but were reachable
only through one small link per card, which is why they read as missing.

`404.html` is generated by `tools/build_404.py` from the same `content/site.json`, because
GitHub Pages serves it for any unmatched path at any depth and its references must be absolute
from the site root. Its copy is ours, not the package's, and it is `noindex`.
