...
**C7 follow-up (2026-09-05), on Delta's "mancano le altre pagine":** the nine docs pages existed and all returned 200, but the only way in was one small "source:" link per card — no nav entry, no footer link, no index. Added `docs/index.html` (generated), a **the pages** nav entry on every page, a footer link, card titles that link, a corrected card label (`read the page`, not `source`), and `404.html` (generated, brand, both languages). `tools/serve.py` now serves locally under the deployed base path so the 404's root-absolute references are checked the way production serves them.
...
- **`sitemap.xml` carries no `<lastmod>`.** Two attempts failed for the same reason: "today" rewrites the file on every run, and the page's last commit date is always one commit stale, because the value is written *into* the commit that changes the page. Both made CI's drift check fail for reasons that were not drift. The element is optional, and the project's rule about the origin applies here too — a value that cannot be made true is worse than no value.
